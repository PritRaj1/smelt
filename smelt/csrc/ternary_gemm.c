...
static inline void build_lut(int16_t a0, int16_t a1, __m256i *vlo, __m256i *vhi) {
    int16_t t[16] = {0};
    t[0] = -a0 - a1; t[1] = -a0;     t[2] = -a0 + a1;
    t[3] = -a1;       t[4] = 0;       t[5] = a1;
    t[6] = a0 - a1;   t[7] = a0;      t[8] = a0 + a1;
    uint8_t lo[16], hi[16];
    for (int i = 0; i < 16; i++) { lo[i] = t[i] & 0xFF; hi[i] = (t[i] >> 8) & 0xFF; }
...
static inline void acc_avx2(__m256i li, __m256i hi, __m256i vlo, __m256i vhi, int16_t *d) {
    ...
    _mm256_storeu_si256((__m256i *)d,       _mm256_add_epi16(_mm256_loadu_si256((__m256i *)d),       _mm256_unpacklo_epi16(ea, oa)));
    _mm256_storeu_si256((__m256i *)(d + 16), _mm256_add_epi16(_mm256_loadu_si256((__m256i *)(d + 16)), _mm256_unpackhi_epi16(ea, oa)));
    _mm256_storeu_si256((__m256i *)(d + 32), _mm256_add_epi16(_mm256_loadu_si256((__m256i *)(d + 32)), _mm256_unpacklo_epi16(eb, ob)));
    _mm256_storeu_si256((__m256i *)(d + 48), _mm256_add_epi16(_mm256_loadu_si256((__m256i *)(d + 48)), _mm256_unpackhi_epi16(eb, ob)));
}
...
static inline void acc_sse(__m128i li, __m128i hi, __m128i vlo, __m128i vhi, int16_t *d) {
    ...
    _mm_storeu_si128((__m128i *)d,       _mm_add_epi16(_mm_loadu_si128((__m128i *)d),       _mm_unpacklo_epi16(ea, oa)));
    _mm_storeu_si128((__m128i *)(d + 8),  _mm_add_epi16(_mm_loadu_si128((__m128i *)(d + 8)),  _mm_unpackhi_epi16(ea, oa)));
    _mm_storeu_si128((__m128i *)(d + 16), _mm_add_epi16(_mm_loadu_si128((__m128i *)(d + 16)), _mm_unpacklo_epi16(eb, ob)));
    _mm_storeu_si128((__m128i *)(d + 24), _mm_add_epi16(_mm_loadu_si128((__m128i *)(d + 24)), _mm_unpackhi_epi16(eb, ob)));
}
...
static inline void flush(int16_t *acc, int32_t *y, int n) {
    ...
    for (int j = 0; j < n; j += 16) {
        ...
        _mm256_storeu_si256((__m256i *)(y + j),     _mm256_add_epi32(_mm256_loadu_si256((__m256i *)(y + j)),     lo));
        _mm256_storeu_si256((__m256i *)(y + j + 8), _mm256_add_epi32(_mm256_loadu_si256((__m256i *)(y + j + 8)), hi));
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < m; i++) {
        ...
        for (int p = 0; p < n_pairs; p++) {
            ...
            if (++sf >= 128) { flush(acc, yi, n_padded); sf = 0; }
        }
